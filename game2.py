...
SHIPS = {"Battleship": [5, "./Sprites/Battleship5.png"],
         ...
         "Fighter Plane": [1, "./Sprites/Plane1.png"]
         }
...
def display_permanent_text():
    ...
    # List of possible enemy grid names
    enemy_grid_name = 'Minh Chau'
    ...
    player_text = grid_header_font.render(f"{username}", True, BLACK, None)
    player_text_rect = player_text.get_rect(center=(240, 60))
    
    enemy_text = grid_header_font.render(enemy_grid_name, True, BLACK, None)
...
def game_over(win: bool):
    ...
    if win:
        ...
        game_over_text = title_font_80.render("Game Over!", True, BLACK, None)
            ...
            if event.type == QUIT:
                ...
                if enemy_grid.rect.collidepoint(event.pos):
                    instruction_text = enemy_cell_clicked(button_list, enemy, enemy_grid, hit_list, player_grid,
                                                          ship_list, instruction_text, wins, loss)
            else:
                pygame.display.update()
...
with open(f"{username}_wins.txt", "r") as f:
    wins = int(f.read())
with open(f"{username}_loss.txt", "r") as f:
    loss = int(f.read())
def enemy_cell_clicked(button_list, enemy, enemy_grid, hit_list, player_grid, ship_list, instruction_text, wins, loss):
                ...
                if ship.name == cell.ship:
                    ...
                    if ship.length == 0:
                        ...
                        if check_for_win(enemy_grid):
                            ...
                            with open (f"{username}_wins.txt","w") as f:
                                f.write('%d' % wins)
                            pygame.time.wait(2000)
                            if game_over(True):
                                ship_list.empty()
                    ...
                    if ship.length == 0:
                        ...
                        if check_for_win(player_grid):
                            ...
                            refresh_screen(player_grid, enemy_grid, button_list, ship_list,
                                           instruction_text, hit_list, colour=RED)
                            loss = loss + 1
                            with open(f'{username}_loss.txt',"w") as f:
                                f.write('%d' % loss)
                            pygame.time.wait(2000)
                            if game_over(False):
                                ship_list.empty()
